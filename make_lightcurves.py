...
if __name__ == "__main__":
    ...
    for f in filtered_mlists:
        ...
        for i, cand in df.iterrows():
            ...
            ax.plot(f_mi_cut['dateobs'].astype(float), f_mi_cut['m'].astype(float), lw=0, c='r', marker='s')
            ax.plot(f_mg_cut['dateobs'].astype(float), f_mg_cut['m'].astype(float), lw=0, c='b', marker='s')
